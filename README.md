...
**Status: Experimental (v0.4.0).** APIs are unstable and may change without notice.
...
| macOS          | planned                                                 |
| Linux          | planned                                                 |
| iOS            | planned (Metal-only, not started)                       |

Windows x64 (Editor and Standalone Player) is fully supported out of the box. Android arm64 support was added in
the C# layer in v0.4.0, but requires you to build and supply the native binary yourself - see
[Android](#android) below. Other platforms are on the roadmap but not implemented yet.
